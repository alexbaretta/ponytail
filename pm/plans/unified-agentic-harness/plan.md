...
- Do not retire or delete the standalone `agent_harness` repository as
  part of this plan.
- Do not migrate project-specific downstream rules into Ponytail.
- Do not migrate a downstream web-client/API parity rule into Ponytail.
...
Web-client/API parity is a downstream-specific architectural rule. Remove it
from the reusable harness material and do not migrate it into Ponytail.
Keep the rule in that project's local customizations.
...
- The harness's API-product client-parity profile was generalized from a
  downstream project and is not a universal API boundary rule.
