...
| Host | Files | Notes |
|------|-------|-------|
| Claude Code | `.claude-plugin/plugin.json`, `commands/`, `hooks/hooks.json`, `hooks/` | Full plugin install with session activation, mode tracking, commands, and statusline support. |
| Codex | `.codex-plugin/plugin.json`, `hooks/hooks.json`, `hooks/`, `skills/` | Plugin install with the same skills plus auto-discovered lifecycle hooks for activation and mode tracking. |
...
| Qoder | `.qoder/rules/ponytail.md`, `.qoder-plugin/plugin.json`, `hooks/qoder-hooks.json`, `skills/`, `AGENTS.md` | Qoder auto-loads `AGENTS.md` as always-on context; `.qoder/rules/ponytail.md` provides per-project rules; the plugin manifest points at the enabled skills in `skills/`. Full plugin-tier: `hooks/qoder-hooks.json` registers prompt and subagent policy injection. Instruction-tier works from the repo root via `AGENTS.md`. |
...
- `skills/ponytail-debt/SKILL.md`: reconcile `tech-debt:` markers into the canonical debt document
