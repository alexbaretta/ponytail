...
description: >-
  Use in repositories that explicitly adopt a project-local
  test_credentials directory when local, integration, browser, acceptance,
  sandbox, cloud, or external-provider testing needs credentials, or before
  concluding that adopted test credentials are unavailable. Discover the
  exact project-user or third-party bundle, enforce environment isolation and
  secret-safe handling, and distinguish credential presence from successful
  validation.
---
