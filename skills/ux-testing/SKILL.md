...
name: ux-testing
description: >-
  Use when diagnosing, implementing, reviewing, or validating a user-facing
  bug or feature in a browser, desktop application, mobile application,
  embedded interface, or other interactive UX. Exercise the real user path
  through the host project's configured UX connection skill, prove the
  observable behavior, and add complementary unit and integration regression
  coverage without test-only product paths.
---
...
2. Use the configured UX connection skill to reproduce the current behavior
   through the real user path.
3. For a bug, apply `debugging` to distinguish the visible symptom from the
   confirmed causal mechanism and identify the owning layer.
...
Direct backend, database, or component probes used by `debugging` remain
supporting evidence. They do not replace proof through the configured UX
connection skill.
