...
description: >-
  Use when a cloud CLI command fails because its configured credentials have
  expired, reauthentication is required, or authentication cannot prompt in
  the current execution context. Recover through the host project's canonical
  reauthentication command, then retry the failed command without changing
  its account, profile, environment, or scope.
---
