...
description: >-
  Use when adding, changing, reading, writing, accepting, returning,
  publishing, persisting, querying, indexing, projecting, migrating, adapting,
  or testing a typed serialized contract that is durable, explicitly
  versioned, or consumed across an independent deployment or external
  compatibility boundary. Applies to database JSON, encrypted JSON plaintext,
  public API requests and responses, events, messages, webhooks, files, and
  projections when those conditions hold. Requires immutable physical
  versions, exhaustive normalization into one current representation,
  latest-only writers, explicit compatibility adapters, and boundary-focused
  verification.
---
